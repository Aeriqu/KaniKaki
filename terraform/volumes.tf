...
resource "kubernetes_persistent_volume_claim_v1" "mongodb-kanji" {
  metadata {
    name      = "mongodb-kanji-volume-claim"
    namespace = kubernetes_namespace_v1.kanikaki.metadata.0.name
  }
  spec {
    volume_name        = "${kubernetes_persistent_volume_v1.mongodb-kanji.metadata.0.name}"
    access_modes       = ["ReadWriteOnce"]
    storage_class_name = "local-storage"
    resources {
      requests = {
        storage = kubernetes_persistent_volume_v1.mongodb-kanji.spec.0.capacity.storage
      }
    }
  }
}

resource "kubernetes_persistent_volume_v1" "mongodb-kanji" {
  metadata {
    name = "mongodb-kanji-volume"
  }
  spec {
    access_modes                     = ["ReadWriteOnce"]
    persistent_volume_reclaim_policy = "Retain"
    ...
    persistent_volume_source {
      host_path {
        path = "/apps/kanikaki/volumes/mongodb-kanji-volume"
        type = "DirectoryOrCreate"
      }
    }
  }
