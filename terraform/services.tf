...
resource "kubernetes_service_v1" "mongodb-srs" {
  metadata {
    name      = "mongodb-srs-service"
    namespace = kubernetes_namespace_v1.kanikaki.metadata.0.name
  }
  spec {
    selector = {
      app = kubernetes_stateful_set_v1.mongodb-srs.metadata.0.labels.app
    }
    port {
      port        = 27017
      target_port = 27017
...
resource "kubernetes_service_v1" "srs" {
  metadata {
    name      = "srs-service"
    namespace = kubernetes_namespace_v1.kanikaki.metadata.0.name
  }
  spec {
    selector = {
      app = kubernetes_deployment_v1.srs.metadata.0.labels.app
    }
    port {
      port        = 80
      target_port = 8080
